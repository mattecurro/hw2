function onJSON(json) {
    ...
    for(sondaggio of json)  {
        ...
        form.setAttribute('action','my_pool');            
        form.setAttribute('method', 'GET');
        const input = document.createElement("input");
        input.setAttribute('type', 'submit');
        input.setAttribute("id", 'vota');
        input.setAttribute('value', 'Andamento');
...
function aggiornaSondaggi() {
    ...
    fetch("pools_logged").then(responseAggiorna).then(onJSON);    
}
...
function cerca(event){
    ...
    fetch("search_logged"+"/"+ cerca).then(responseAggiorna).then(onJSON);  
